...
it("initializes silent startup from settings query and sends it in mutation payload", async () => {
  vi.mocked(useSettingsQuery).mockReturnValue({
    data: createSettings({ auto_start: true, silent_startup: true, tray_enabled: true }),
    isLoading: false,
    isError: false,
    error: null,
  } as any);
  ...
  mutation.mutateAsync.mockResolvedValue(
    createSettings({ auto_start: true, silent_startup: false, tray_enabled: true })
  );
  ...
  expect(result.current.silentStartup).toBe(true);

  act(() => {
    result.current.requestPersist({ silent_startup: false });
  });

  await waitFor(() => expect(mutation.mutateAsync).toHaveBeenCalled());
  expect(mutation.mutateAsync).toHaveBeenCalledWith(
    expect.objectContaining({ silentStartup: false })
  );
});
...
it("toasts circuit parameters note when saved while gateway running", async () => {
  ...
  const mutation = { mutateAsync: vi.fn() };
  mutation.mutateAsync.mockResolvedValue(createSettings({ circuit_breaker_failure_threshold: 6 }));
  vi.mocked(useSettingsSetMutation).mockReturnValue(mutation as any);
